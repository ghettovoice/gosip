...
func ParseSIPURI[T ~string | ~[]byte](s T) (*abnf.Node, error) {
	if len(s) == 0 {
		return nil, errors.Wrap(ErrEmptyInput)
	}
	...
	if err := rfc3261.Rules().SIPURI([]byte(s), ns); err != nil {
		return nil, errors.Wrap(newMalformedInputError(err))
	}

	n := ns.Best()
	if nl, il := n.Len(), len(s); nl < il {
		return nil, errors.Wrap(newMalformedInputError("parsed length %d < input length %d", nl, il))
	}

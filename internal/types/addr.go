...
func ParseAddr[T ~string | ~[]byte](s T) (addr Addr, err error) {
	...
	return errors.Wrap2(AddrFromABNF(node))
}
...
func (addr Addr) IsValid() bool {
	return grammar.IsHost(addr.host) && (!addr.hasPort || addr.port > 0)
}

// IsZero reports whether the address has zero host, IP and port information.
func (addr Addr) IsZero() bool {
	return addr.host == "" && addr.ip == nil && !addr.hasPort
}
...
func (addr Addr) WithPort(port uint16) Addr {
	addr.ip = slices.Clone(addr.ip)
	addr.port = port
	addr.hasPort = true
	return addr
...
func (addr Addr) WithPortIfMissing(port uint16) Addr {
	if !addr.hasPort {
		return addr.WithPort(port)
	}
	return addr
}

func (addr Addr) WithoutPort() Addr {
	addr.ip = slices.Clone(addr.ip)
	addr.hasPort = false
	addr.port = 0
	return addr

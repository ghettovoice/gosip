// Package timeutil provides [Timer], a drop-in replacement for [time.AfterFunc] that
// can be snapshotted, serialized, and restored for long-lived workflows such as SIP transaction timeouts.
//
// A [Timer] keeps its runtime behaviour—automatic callback execution via a background
// time.Timer—while exposing deterministic state through [TimerSnapshot].
// Snapshots can be marshaled to JSON, stored, and later passed to [RestoreTimer]
// to obtain a fresh timer instance.
// Callbacks are runtime-only; they must be reattached after restoration with [Timer.SetCallback] or [Timer.Reset].
//
// Basic usage:
//

...
func (u *Any) String() string { return u.Render() }
...
func (u *Any) Equal(val any) bool {
	...
	return util.EqFold(u.Render(), other.Render())
}
...
// ParseAny parses an arbitrary URI from the given input (string or []byte).
func ParseAny[T ~string | ~[]byte](s T) (u *Any, err error) {
	if len(s) == 0 {
		return nil, errors.Wrap(grammar.ErrEmptyInput)
	}
	...
	nu, err := url.Parse(string(s))
	if err != nil {
		return nil, errors.Wrap(errors.Prefix(grammar.ErrMalformedInput, err))
	}

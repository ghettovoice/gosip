...
func (*Tel) Scheme() string { return "tel" }
...
func (u *Tel) String() string { return u.Render() }
...
func (u *Tel) UnmarshalText(text []byte) error {
	if len(text) == 0 || bytes.EqualFold(text, []byte("tel:")) {
		*u = Tel{}
		return nil
	}
...
// ParseTel parses a Tel URI from the given input (string or []byte).
func ParseTel[T ~string | ~[]byte](s T) (u *Tel, err error) {
	defer func() {

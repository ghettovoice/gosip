...
func (hdr *customHeader) Render(...header.RenderOptions) string {
	if hdr == nil {
		return ""
	}
		...
		},
		{
			...
			func() error {
				return testAllHeaderElems[header.Supported](
					...
					"Supported",
					"header.Supported",
					func() []*header.OptionTag {
						ptrs := make([]*header.OptionTag, 3)
						for i, el := range []header.OptionTag{"opt1", "opt2", "opt3"} {
							ptrs[i] = &el
						}

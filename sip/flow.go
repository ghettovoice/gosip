...
func init() {
	// TODO: expose public package-level functions to init custom secret
...
func (t FlowToken) MarshalText() ([]byte, error) {
	if !t.IsValid() {
		return nil, errors.ErrorWrap("invalid flow token")
	}
...
func (t FlowToken) AppendText(b []byte) ([]byte, error) {
	if !t.IsValid() {
		return b, errors.ErrorWrap("invalid flow token")
	}
...
func (t *FlowToken) UnmarshalText(data []byte) error {
	...
	const hmacSize = 10
	if len(raw) <= hmacSize {
		return errors.ErrorfWrap("invalid token length %d", len(raw))
	}
	...
	if !hmac.Equal(macPart, sum[:hmacSize]) {
		return errors.ErrorWrap("invalid token signature")
	}

	token, ok := parseFlowTokenPayload(payload)
	if !ok {
		return errors.ErrorWrap("invalid token payload")
	}

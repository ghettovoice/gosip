...
var _ Parser = (*StdParser)(nil)

func (p *StdParser) maxMsgSize() uint {
	if p.MaxMessageSize == 0 {
		return uint(math.MaxUint16)
	}
...
func (p *StdParser) ParsePacket(b []byte) (Message, error) {
	...
	return errors.Wrap2(parseMsg(br, true, p.maxMsgSize()))
}
...
func (p *StdParser) ParseStream(rdr io.Reader) StreamParser {
	return &StdStreamParser{rdr: rdr, maxMsgSize: p.maxMsgSize()}
}
...
var _ StreamParser = (*StdStreamParser)(nil)
...
func (p *StdStreamParser) Messages() iter.Seq2[Message, error] {
	return func(yield func(Message, error) bool) {
		...
		for {
			msg, err := parseMsg(br, false, p.maxMsgSize)

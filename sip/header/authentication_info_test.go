...
func TestAuthenticationInfo_MarshalJSON(t *testing.T) {
	t.Parallel()
	...
	cases := []struct {
		...
		{"nil", nil, "null"},
		{"zero", &header.AuthenticationInfo{}, `{"name":"Authentication-Info","value":""}`},
		{
			"full",
			&header.AuthenticationInfo{
				NextNonce:  "qwerty",
			...
			`{"name":"Authentication-Info","value":"cnonce=\"1q2w3e\", nc=00000005, nextnonce=\"qwerty\", qop=auth-int, rspauth=\"abcdef\""}`,
		},
	}
...
func TestAuthenticationInfo_UnmarshalJSON(t *testing.T) {
	t.Parallel()

	cases := []struct {
		name    string
		data    string
		want    *header.AuthenticationInfo
		wantErr bool
	}{
		{"null", "null", nil, false},
		{"empty object", `{}`, nil, true},
		{"empty name", `{"value":"cnonce=\"1q2w3e\", nc=00000005, nextnonce=\"qwerty\", qop=auth-int, rspauth=\"abcdef\""}`, nil, true},
		{"empty value", `{"name":"Authentication-Info","value":""}`, &header.AuthenticationInfo{}, false},
		{"wrong header", `{"name":"From","value":"<sip:alice@example.com>"}`, nil, true},
		{"invalid json", `{"name":"Authentication-Info","value":`, nil, true},
		{
			"full",
			`{"name":"Authentication-Info","value":"cnonce=\"1q2w3e\", nc=00000005, nextnonce=\"qwerty\", qop=auth-int, rspauth=\"abcdef\""}`,
			&header.AuthenticationInfo{
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			var got *header.AuthenticationInfo
...
func TestAuthenticationInfo_RoundTripJSON(t *testing.T) {
	t.Parallel()

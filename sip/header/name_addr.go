...
func (addr NameAddr) String() string {
	...
	fmt.Fprint(sb, "<")
	if addr.URI != nil {
		addr.URI.RenderTo(sb)
	}
	fmt.Fprint(sb, ">")

	_, _ = renderHdrParams(sb, addr.Params, false)
	return sb.String()
}
...
func (addr NameAddr) Format(f fmt.State, verb rune) {
	...
	default:
		...
		type (
			hideMethods NameAddr
			NameAddr    hideMethods
		)
...
func (addr NameAddr) AppendText(data []byte) ([]byte, error) {
	return append(data, addr.String()...), nil
}

func (addr *NameAddr) UnmarshalText(data []byte) (finErr error) {
...
func buildFromNameAddrNode(node *abnf.Node, psNodeKey string) NameAddr {
	addr := NameAddr{
		URI:    util.Must2(uri.FromABNF(grammar.MustGetNode(node, "addr-spec").Children[0])),

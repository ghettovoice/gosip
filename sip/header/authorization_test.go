package header_test

import (
	"reflect"

	. "github.com/onsi/ginkgo/v2"
	. "github.com/onsi/gomega"

	"github.com/ghettovoice/gosip/sip/header"
	"github.com/ghettovoice/gosip/sip/uri"
)

var _ = Describe("Header", Label("sip", "header"), func() {
	Describe("Authorization", func() {
		assertHeaderParsing(
			// region
			Entry(nil, "Authorization: qwerty", &header.Any{Name: "Authorization", Value: "qwerty"}, nil),
			Entry(nil,
				"Authorization: Digest username=\"root\", realm=\"example.com\", nonce=\"qwerty\",\r\n"+
					"\turi=\"sip:example.com\", response=\"587245234b3434cc3412213e5f113a54\", algorithm=MD5,\r\n"+
					"\tcnonce=\"1q2w3e\", opaque=\"zxc\", qop=auth, nc=00000005, p1=abc, p2=\"a b c\"",
				&header.Authorization{
					AuthCredentials: &header.DigestCredentials{
						Username:   "root",
						Realm:      "example.com",
						Nonce:      "qwerty",
						URI:        &uri.SIP{Addr: uri.Host("example.com")},
						Response:   "587245234b3434cc3412213e5f113a54",
						Algorithm:  "MD5",
						CNonce:     "1q2w3e",
						Opaque:     "zxc",
						QOP:        "auth",
						NonceCount: 5,
						Params:     make(header.Values).Set("p1", "abc").Set("p2", `"a b c"`),
					},
				},
				nil,
			),
			Entry(nil,
				"Authorization: Bearer QweRTY123",
				&header.Authorization{
					AuthCredentials: &header.BearerCredentials{
						Token: "QweRTY123",
					},
				},
				nil,
			),
			Entry(nil,
				"Authorization: Custom p1=abc, p2=\"a b c\"",
				&header.Authorization{
					AuthCredentials: &header.AnyCredentials{
						Scheme: "Custom",
						Params: make(header.Values).Set("p1", "abc").Set("p2", `"a b c"`),
					},
				},
				nil,
			),
			// endregion
		)

		assertHeaderRendering(
			// region
			Entry(nil, (*header.Authorization)(nil), ""),
			Entry(nil, &header.Authorization{}, "Authorization: "),
			Entry(nil,
				&header.Authorization{
					AuthCredentials: &header.DigestCredentials{
						Username:   "root",
						Realm:      "example.com",
						Nonce:      "qwerty",
						URI:        &uri.SIP{Addr: uri.Host("example.com")},
						Response:   "587245234b3434cc3412213e5f113a54",
						Algorithm:  "MD5",
						CNonce:     "1q2w3e",
						Opaque:     "zxc",
						QOP:        "auth",
						NonceCount: 5,
						Params:     make(header.Values).Set("p1", "abc").Set("p2", `"a b c"`),
					},
				},
				"Authorization: Digest algorithm=MD5, cnonce=\"1q2w3e\", nc=00000005, nonce=\"qwerty\", opaque=\"zxc\", "+
					"qop=auth, realm=\"example.com\", response=\"587245234b3434cc3412213e5f113a54\", username=\"root\", "+
					"uri=\"sip:example.com\", p1=abc, p2=\"a b c\"",
			),
			Entry(nil,
				&header.Authorization{
					AuthCredentials: &header.BearerCredentials{
						Token: "QweRTY123",
					},
				},
				"Authorization: Bearer QweRTY123",
			),
			Entry(nil,
				&header.Authorization{
					AuthCredentials: &header.AnyCredentials{
						Scheme: "Custom",
						Params: make(header.Values).Set("p1", "abc").Set("p2", `"a b c"`),
					},
				},
				"Authorization: Custom p1=abc, p2=\"a b c\"",
			),
			// endregion
		)

		assertHeaderComparing(
			// region
			Entry(nil, (*header.Authorization)(nil), nil, false),
			Entry(nil, (*header.Authorization)(nil), (*header.Authorization)(nil), true),
			Entry(nil, &header.Authorization{}, (*header.Authorization)(nil), false),
			Entry(nil,
				&header.Authorization{
					AuthCredentials: &header.DigestCredentials{
						Username:   "root",
						Realm:      "example.com",
						Nonce:      "qwerty",
						URI:        &uri.SIP{Addr: uri.Host("example.com")},
						Response:   "587245234b3434cc3412213e5f113a54",
						Algorithm:  "MD5",
						CNonce:     "1q2w3e",
						Opaque:     "zxc",
						QOP:        "auth",
						NonceCount: 5,
						Params:     make(header.Values).Set("p1", "abc").Set("p2", `"a b c"`),
					},
				},
				header.Authorization{
					AuthCredentials: &header.DigestCredentials{
						Username:   "root",
						Realm:      "example.com",
						Nonce:      "qwerty",
						URI:        &uri.SIP{Addr: uri.Host("example.com")},
						Response:   "587245234b3434cc3412213e5f113a54",
						Algorithm:  "md5",
						CNonce:     "1q2w3e",
						Opaque:     "zxc",
						QOP:        "AUTH",
						NonceCount: 5,
						Params:     make(header.Values).Set("p1", "ABC").Set("p2", `"a b c"`),
					},
				},
				true,
			),
			Entry(nil,
				&header.Authorization{
					AuthCredentials: &header.DigestCredentials{
						Username:   "root",
						Realm:      "example.com",
						Nonce:      "qwerty",
						URI:        &uri.SIP{Addr: uri.Host("example.com")},
						Response:   "587245234b3434cc3412213e5f113a54",
						Algorithm:  "MD5",
						CNonce:     "1q2w3e",
						Opaque:     "zxc",
						QOP:        "auth",
						NonceCount: 5,
						Params:     make(header.Values).Set("p1", "abc").Set("p2", `"a b c"`),
					},
				},
				&header.Authorization{
					AuthCredentials: &header.DigestCredentials{
						Username:   "ROOT",
						Realm:      "example.com",
						Nonce:      "qwerty",
						URI:        &uri.SIP{Addr: uri.Host("example.com")},
						Response:   "587245234b3434cc3412213e5f113a54",
						Algorithm:  "MD5",
						CNonce:     "1q2w3e",
						Opaque:     "zxc",
						QOP:        "auth",
						NonceCount: 5,
						Params:     make(header.Values).Set("p1", "abc").Set("p2", `"a b c"`),
					},
				},
				false,
			),
			Entry(nil,
				&header.Authorization{
					AuthCredentials: &header.DigestCredentials{
						Username:   "root",
						Realm:      "example.com",
						Nonce:      "qwerty",
						URI:        &uri.SIP{Addr: uri.Host("example.com")},
						Response:   "587245234b3434cc3412213e5f113a54",
						Algorithm:  "MD5",
						CNonce:     "1q2w3e",
						Opaque:     "zxc",
						QOP:        "auth",
						NonceCount: 5,
						Params:     make(header.Values).Set("p1", "abc").Set("p2", `"a b c"`),
					},
				},
				&header.Authorization{
					AuthCredentials: (*header.DigestCredentials)(nil),
				},
				false,
			),
			Entry(nil,
				&header.Authorization{
					AuthCredentials: &header.DigestCredentials{
						Username:   "root",
						Realm:      "example.com",
						Nonce:      "qwerty",
						URI:        &uri.SIP{Addr: uri.Host("example.com")},
						Response:   "587245234b3434cc3412213e5f113a54",
						Algorithm:  "MD5",
						CNonce:     "1q2w3e",
						Opaque:     "zxc",
						QOP:        "auth",
						NonceCount: 5,
						Params:     make(header.Values).Set("p1", "abc").Set("p2", `"a b c"`),
					},
				},
				&header.Authorization{
					AuthCredentials: &header.BearerCredentials{
						Token: "QwertY",
					},
				},
				false,
			),
			Entry(nil,
				&header.Authorization{
					AuthCredentials: &header.BearerCredentials{
						Token: "QWERTY",
					},
				},
				&header.Authorization{
					AuthCredentials: &header.BearerCredentials{
						Token: "qwerty",
					},
				},
				false,
			),
			Entry(nil,
				&header.Authorization{
					AuthCredentials: &header.BearerCredentials{
						Token: "QwertY",
					},
				},
				&header.Authorization{
					AuthCredentials: &header.BearerCredentials{
						Token: "QwertY",
					},
				},
				true,
			),
			Entry(nil,
				&header.Authorization{
					AuthCredentials: &header.AnyCredentials{
						Scheme: "custom",
						Params: make(header.Values).Set("p1", "abc").Set("p2", `"a b c"`),
					},
				},
				&header.Authorization{
					AuthCredentials: &header.AnyCredentials{
						Scheme: "Custom",
						Params: make(header.Values).Set("p1", "ABC").Set("p2", `"a b c"`),
					},
				},
				true,
			),
			Entry(nil,
				&header.Authorization{
					AuthCredentials: &header.AnyCredentials{
						Scheme: "Custom",
						Params: make(header.Values).Set("p1", "abc").Set("p2", `"a b c"`),
					},
				},
				&header.Authorization{
					AuthCredentials: &header.AnyCredentials{
						Scheme: "Qwerty",
						Params: make(header.Values).Set("p1", "abc").Set("p2", `"a b c"`),
					},
				},
				false,
			),
			Entry(nil,
				&header.Authorization{
					AuthCredentials: &header.AnyCredentials{
						Scheme: "Custom",
						Params: make(header.Values).Set("p1", "abc").Set("p2", `"a b c"`),
					},
				},
				&header.Authorization{
					AuthCredentials: &header.AnyCredentials{
						Scheme: "Custom",
						Params: make(header.Values).Set("p1", "abc").Set("p2", `"zxc"`),
					},
				},
				false,
			),
			// endregion
		)

		assertHeaderValidating(
			// region
			Entry(nil, (*header.Authorization)(nil), false),
			Entry(nil, &header.Authorization{}, false),
			Entry(nil,
				&header.Authorization{
					AuthCredentials: &header.DigestCredentials{
						Username: "root",
						Response: "587245234b3434cc3412213e5f113a54",
					},
				},
				false,
			),
			Entry(nil,
				&header.Authorization{
					AuthCredentials: &header.DigestCredentials{
						Username:  "root",
						Realm:     "example.com",
						Nonce:     "qwerty",
						URI:       &uri.SIP{Addr: uri.Host("example.com")},
						Response:  "587245234b3434cc3412213e5f113a54",
						Algorithm: "MD5",
						QOP:       "auth",
					},
				},
				true,
			),
			Entry(nil,
				&header.Authorization{
					AuthCredentials: &header.BearerCredentials{},
				},
				false,
			),
			Entry(nil,
				&header.Authorization{
					AuthCredentials: &header.BearerCredentials{Token: "QwertY"},
				},
				true,
			),
			Entry(nil,
				&header.Authorization{
					AuthCredentials: &header.AnyCredentials{},
				},
				false,
			),
			Entry(nil,
				&header.Authorization{
					AuthCredentials: &header.AnyCredentials{
						Scheme: "Custom",
						Params: make(header.Values),
					},
				},
				false,
			),
			Entry(nil,
				&header.Authorization{
					AuthCredentials: &header.AnyCredentials{
						Scheme: "Custom",
						Params: make(header.Values).Set("p1", "abc"),
					},
				},
				true,
			),
			// endregion
		)

		assertHeaderCloning(
			// region
			func(hdr1, hdr2 *header.Authorization) {
				Expect(reflect.ValueOf(hdr2).Pointer()).ToNot(Equal(reflect.ValueOf(hdr1).Pointer()))
				switch crd1 := hdr1.AuthCredentials.(type) {
				case *header.DigestCredentials:
					crd2, _ := hdr2.AuthCredentials.(*header.DigestCredentials)
					if crd1 == nil || reflect.ValueOf(crd1).IsNil() {
						Expect(crd2).To(BeNil())
					} else {
						Expect(reflect.ValueOf(crd2).Pointer()).
							ToNot(Equal(reflect.ValueOf(crd1).Pointer()))
						if crd1.URI == nil || reflect.ValueOf(crd1.URI).IsNil() {
							Expect(crd2.URI).To(BeNil())
						} else {
							Expect(reflect.ValueOf(crd2.URI).Pointer()).
								ToNot(Equal(reflect.ValueOf(crd1.URI).Pointer()))
						}
						if crd1.Params == nil {
							Expect(crd2.Params).To(BeNil())
						} else {
							Expect(reflect.ValueOf(crd2.Params).Pointer()).
								ToNot(Equal(reflect.ValueOf(crd1.Params).Pointer()))
						}
					}
				case *header.BearerCredentials:
					crd2, _ := hdr2.AuthCredentials.(*header.BearerCredentials)
					if crd1 == nil || reflect.ValueOf(crd1).IsNil() {
						Expect(crd2).To(BeNil())
					} else {
						Expect(reflect.ValueOf(crd2).Pointer()).ToNot(Equal(reflect.ValueOf(crd1).Pointer()))
					}
				case *header.AnyCredentials:
					crd2, _ := hdr2.AuthCredentials.(*header.AnyCredentials)
					if crd1 == nil || reflect.ValueOf(crd1).IsNil() {
						Expect(crd2).To(BeNil())
					} else {
						Expect(reflect.ValueOf(crd2).Pointer()).ToNot(Equal(reflect.ValueOf(crd1).Pointer()))
						if crd1.Params == nil {
							Expect(crd2.Params).To(BeNil())
						} else {
							Expect(reflect.ValueOf(crd2.Params).Pointer()).
								ToNot(Equal(reflect.ValueOf(crd1.Params).Pointer()))
						}
					}
				}
			},
			Entry(nil, (*header.Authorization)(nil)),
			Entry(nil, &header.Authorization{}),
			Entry(nil,
				&header.Authorization{
					AuthCredentials: &header.DigestCredentials{
						Username:   "root",
						Realm:      "example.com",
						Nonce:      "qwerty",
						URI:        &uri.SIP{Addr: uri.Host("example.com")},
						Response:   "587245234b3434cc3412213e5f113a54",
						Algorithm:  "MD5",
						CNonce:     "1q2w3e",
						Opaque:     "zxc",
						QOP:        "auth",
						NonceCount: 5,
						Params:     make(header.Values).Set("p1", "abc").Set("p2", `"a b c"`),
					},
				},
			),
			Entry(nil,
				&header.Authorization{
					AuthCredentials: &header.BearerCredentials{
						Token: "QweRTY123",
					},
				},
			),
			Entry(nil,
				&header.Authorization{
					AuthCredentials: &header.AnyCredentials{
						Scheme: "Custom",
						Params: make(header.Values).Set("p1", "abc").Set("p2", `"a b c"`),
					},
				},
			),
			// endregion
		)
	})
})

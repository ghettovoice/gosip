...
func TestContentType_Render(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			[]header.RenderOptions{{Compact: true}},
...
func TestContentType_MarshalJSON(t *testing.T) {
	t.Parallel()
	...
	cases := []struct {
		...
		{"nil", nil, "null"},
		{"zero", &header.ContentType{}, `{"name":"Content-Type","value":"/"}`},
		{
			"full",
			&header.ContentType{
				Type:    "application",
			...
			`{"name":"Content-Type","value":"application/sdp;charset=UTF-8"}`,
		},
	}
...
func TestContentType_UnmarshalJSON(t *testing.T) {
	...
	cases := []struct {
		name    string
		data    string
		want    *header.ContentType
		wantErr bool
	}{
		{"null", "null", nil, false},
		{"empty object", `{}`, nil, true},
		{"empty name", `{"value":"text/plain"}`, nil, true},
		{"empty value", `{"name":"Content-Type","value":""}`, &header.ContentType{}, false},
		{"wrong header", `{"name":"From","value":"<sip:alice@example.com>"}`, nil, true},
		{"invalid json", `{"name":"Content-Type","value":`, nil, true},
		{"invalid value", `{"name":"Content-Type","value":"application"}`, nil, true},
		{"zero", `{"name":"Content-Type","value":"/"}`, &header.ContentType{}, false},
		{
			"full",
			`{"name":"Content-Type","value":"application/sdp;charset=UTF-8"}`,
			&header.ContentType{
...
func TestContentType_RoundTripJSON(t *testing.T) {
	t.Parallel()

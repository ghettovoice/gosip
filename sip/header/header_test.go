	...
	cases := []struct {
...

		{
			...
			map[string]header.Parser{
				"x-custom": func(name string, value []byte) (header.Header, error) {
					return &customHeader{Name: name, Value: value}, nil
				},
			},
		...
		{"retry-after 3", "Retry-After: 120", nil, &header.RetryAfter{Delay: 120 * time.Second}, nil},
		{
			"retry-after 4",
		...
		{"timestamp 2", "Timestamp: 0", nil, &header.Timestamp{RequestTime: time.Unix(0, 0).UTC()}, nil},
		{"timestamp 3", "Timestamp: 0.543", nil, &header.Timestamp{RequestTime: time.Unix(0, 543*1e6).UTC()}, nil},
		{
			"timestamp 4",
...
func (h *customHeader) RenderTo(w io.Writer, _ *header.RenderOptions) (int, error) {
	return w.Write([]byte(h.RenderValue()))
}

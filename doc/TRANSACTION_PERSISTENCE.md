...
The recommended approach is to use `BindStateHandler` callback to automatically persist transactions:
...
tx.BindStateHandler(func(ctx context.Context, from, to sip.TransactionState) {
    snapshot := tx.Snapshot()
    data, _ := json.Marshal(snapshot)
...
❌ Error callbacks (`BindErrorHandler`) (need to be re-registered)  
❌ ACK callbacks (`BindAckHandler`) for INVITE server transactions (need to be re-registered)
❌ Response callbacks (`BindResponseHandler`) for client transactions (need to be re-registered)  
...
for _, snapshot := range snapshots {
    ...
    tx.BindErrorHandler(func(ctx context.Context, err error) {
        // Handle errors
    })
    // For INVITE transactions
    tx.BindAckHandler(func(ctx context.Context, ack *sip.Request) {
        // Handle 2xx ACK
    })
}
```
...
4. **Error callbacks (`BindErrorHandler`)** - Must be re-registered after restoration
